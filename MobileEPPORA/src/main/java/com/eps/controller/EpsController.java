...
public class EpsController {
	...
	ApplicationContext EbEpsContext = new ClassPathXmlApplicationContext(
			"ebeps.xml");
	ApplicationContext DbEpsContext = new ClassPathXmlApplicationContext(
			"dbeps.xml");
	...
	public ModelAndView forgotPassword(
			@RequestParam(value = "stEmail", required = false, defaultValue = "") String stEmail)
			throws SQLException {
		/*
		...
		ResultSet rsUser = this.Ebdb.ExecuteSql("select * from Users u, "
				+ this.dbDyn.getDbName() + ".X25User xu where "
		...
		this.dbDyn.ExecuteUpdate("update x25user set stPassword=password('"
		...
		int iRecId = 0;
		try {
			iRecId = ebM.sendMail(stEmail, rsUser.getString("FirstName") + " "
	...
	public ModelAndView LoginProcess(
			@RequestParam(value = "stEmail", required = false, defaultValue = "") String stEmail,
			@RequestParam(value = "stPwd", required = false, defaultValue = "") String stPwd,
			HttpSession session) {
		/*
		 * users click on "Login" and come to this controller.
		 */

		EbClientTemplate EbcTemplate = (EbClientTemplate) EbEpsContext
				.getBean("EbClientTemplate");
		...
		if (EbC.isEmpty()) {
			 ...
			 * check logged in
			 */
			EbClient ebClient = EbC.get(0);
			if (ebClient.getNmSuccessLoginTime() > 0) {
				ModelAndView mav = new ModelAndView("welcome");
				mav.addObject("popupMessage",
						"E-Mail/Password is being used. <br>Please try again after "
			 ...
			 * get the previlage.
			 */
			EbUserDataTemplate EbUDTemplate = (EbUserDataTemplate) DbEpsContext
					.getBean("EbUserDataTemplate");
			List<EbUserData> EbUD = EbUDTemplate.mappingUserData(ebClient);
			...
			if (client.getStPriviledge().contains("Pm")
					|| client.getStPriviledge().contains("Ppm")) {
	...
	public ModelAndView getWorkFlow(HttpSession session) {
		...
		EbWorkFlowTemplate EbWFTemplate = (EbWorkFlowTemplate) DbEpsContext
				.getBean("EbWorkFlowTemplate");
		EbWF = EbWFTemplate.mappingWorkFlow(EbUD, DbEpsContext);
		List<EbWorkFlow> tempEbWf = EbWFTemplate.mappingWorkFlow(EbUD,
				DbEpsContext);
	...
	public ModelAndView getMessage(HttpSession session) {
		...
		EbMessagesTemplate EbMsgTemplate = (EbMessagesTemplate) EbEpsContext
				.getBean("EbMessagesTemplate");
	...
	public ModelAndView getWFofPTM() {
		...
		EbWorkFlowOfPTMTemplate EbWFofPTMTemplate = (EbWorkFlowOfPTMTemplate) DbEpsContext
				.getBean("EbWorkFlowOfPTMTemplate");
	...
	public ModelAndView getDashboard() {
		...
		EbExecutiveDashboardTemplate EbDashboardTemplate = (EbExecutiveDashboardTemplate) DbEpsContext
				.getBean("EbExecutiveDashboardTemplate");
	...
	// public ModelAndView workFlowOfPTMEdit(
	// @RequestParam("expendedHoursToday") String stEHT,
	// @RequestParam(value="incrementalProgress",required=false,
	// defaultValue="") String stProg, @RequestParam("status") String status,
	// @RequestParam(value="message",required=false, defaultValue="") String
	...
	public ModelAndView ptmReject(
			@RequestParam(value = "expendedHoursToday", required = false, defaultValue = "") String stEHT,
			@RequestParam(value = "incrementalProgress", required = false, defaultValue = "") String stProg,
		...
		String stWhere = " WHERE nmProjectId=" + ptmTemp.getPrjId()
				+ " AND nmBaseline=" + ptmTemp.getBaseline() + " AND RecId="
				+ iRecId;
		...
		try {
			...
			double nmTodaySpent = this.Ebdb
					.ExecuteSql1nm("SELECT SUM(nmActualApproved)"
							...
							+ this.Ebdb.fmtDbString(String.valueOf(EbUD
									.getNmUserId()))
			...
			String msg = "Task " + stSchId + ": Estimated \""
					...
					+ ptmTemp.getExpendedHoursToday() + "\", Status \""
					+ this.Ebdb.fmtDbString(ptmTemp.getStatus()) + "\"";
			makeMessage(
					rsP.getString("ProjectName"),
					String.valueOf(this.EbUD.getNmUserId()),
					"A Workflow Task update is rejected",
					this.Ebdb.fmtDbString(msg),
					dateEnd,
					"./?stAction=projects&t=12&do=xls&pk="
							+ this.Ebdb.fmtDbString(ptmTemp.getPrjId())
							+ "&parent=&child=21&a=editfull&r=" + iRecId);

			this.Ebdb.ExecuteUpdate("DELETE FROM teb_workflow" + stWhere);
			...
			epsXlsProject.recalcSchedule("" + iRecId, stPrjId,
	...
	public ModelAndView ptmUpdate(
		...
		try {
			...
			if (ptmTemp.getPgAccomplishedToDate() != null
					&& !ptmTemp.getPgAccomplishedToDate().equalsIgnoreCase(""))
		...
		ptmTemp.getEbPg().setPlannedQuantity(
				String.valueOf(dIncrementalProgress));
		ptmTemp.setStatus(status);
		ptmTemp.setMessage(message);
		...
		String stAllocateWhere = " WHERE nmPrjId=" + stPrjId
				+ " AND nmBaseline=" + stBaseline + " AND nmTaskID=" + stRecId;
		String stWhere = " WHERE nmProjectId=" + ptmTemp.getPrjId()
				+ " AND nmBaseline=" + ptmTemp.getBaseline() + " AND RecId="
				+ stRecId;
		String stSchId = ptmTemp.getSchId();
		...
		try {
			...
			double nmTodaySpent = this.Ebdb
					.ExecuteSql1nm("SELECT SUM(nmActualApproved)"
							+ " FROM teb_allocateprj WHERE nmUserId="
							+ EbUD.getNmUserId() + " AND dtDatePrj=curdate()");
			double nmTodayExpended = this.Ebdb
					.ExecuteSql1nm("SELECT SUM(nmActual)"
							+ " FROM teb_workflow WHERE nmUserId="
							+ EbUD.getNmUserId() + " AND dtAllocate=curdate()");
			int iMaxHoursPerDay = 10; // this.rsMyDiv.getInt("MaxWorkHoursPerDay");
				...
				if (addedUp > 0)
					this.Ebdb
							.ExecuteUpdate("UPDATE schedule_progress SET nmAccomplished=nmAccomplished+"
									+ this.Ebdb.fmtDbString(String
											.valueOf(addedUp))
									+ " WHERE RecId="
									+ ptmTemp.getEbPg().getProgressId());
			}

			ResultSet rs = this.Ebdb.ExecuteSql("SELECT * FROM teb_allocateprj"
			...
			this.Ebdb.ExecuteUpdate("UPDATE Schedule set SchStatus="
					...
					+ stWhere);
			EpsXlsProject epsXlsProject = new EpsXlsProject();
			epsXlsProject.recalcSchedule(stRecId, stPrjId,
					Integer.parseInt(stBaseline));
			...
			makeTask(15, ptmTemp.getSchTitle());
			// TODO: dunghm Milestone messages
			sendMilestoneMessage(stPrjId, Integer.parseInt(stBaseline), stRecId);
	...
	public ModelAndView workFlowEdit(
			@RequestParam("expendedHoursToday") String stEHT,
			@RequestParam("incrementalProgress") String stProg,
		...
		double dExpendedHoursToday;
		double dIncrementalProgress = 0;
		EbWorkFlow wfTemp = this.editWF; // This is the entry the user just
											// clicked
		try {
			dExpendedHoursToday = Double.parseDouble(stEHT);
		...
		/*
		 * Work Flow class has following fields to contain proper database
		 * information in it. String : prjId, baseline, schId, projectName,
		 * startDate, expectedHoursToDate; estimatedHours, expendedHoursToday
		 * ,status, description, message, pgDescription(progress),
		 * pgPlannedQuantity(progress), pgIncrementalProgress(progress),
		 * PgAccomplishedToDate(progress), Int taskId those fields are filled
		 * when the user first click on "work flow"menu on their home page, via
		 * Spring template.
		 */

		/*
		 * EbUD is EbUserData class which contains e-mails, recId, username,
		 * etc. Things that are related to users. Except for the parameters,
		 * code is just same as eps project.
		 */
		String stWhere = " WHERE nmProjectId="
				+ this.Ebdb.fmtDbString(wfTemp.getPrjId()) + " AND nmBaseline="
				+ this.Ebdb.fmtDbString(wfTemp.getBaseline()) + " AND RecId="
				+ wfTemp.getTaskId();
		String stSchId = wfTemp.getSchId();

		ResultSet rsP = this.Ebdb
				.ExecuteSql("SELECT * FROM Projects WHERE RecId="
						+ this.Ebdb.fmtDbString(wfTemp.getPrjId()));
		rsP.next();
		int iCount = 0;
		...
		try {
			...
			//
			// stSql =
			// "REPLACE INTO schedule_progress_allocate (nmProgressId,nmUserId,dtUpdate,nmActual) VALUES ";
			//
			...
			this.Ebdb.ExecuteUpdate("REPLACE INTO teb_workflow VALUE("
					+ this.Ebdb.fmtDbString(wfTemp.getPrjId()) + ","
					+ this.Ebdb.fmtDbString(wfTemp.getBaseline()) + ","
					+ wfTemp.getTaskId() + ","
					+ this.Ebdb.fmtDbString(wfTemp.getEstimatedHours()) + ","
					+ this.Ebdb.fmtDbString(wfTemp.getExpendedHoursToday())
					+ "," + 0 + ","
			...
			String msg = "Task " + stSchId + ": Estimated \""
					...
					+ wfTemp.getExpendedHoursToday() + "\", Status \""
					+ this.Ebdb.fmtDbString(wfTemp.getStatus()) + "\"";
			makeMessage(
					rsP.getString("ProjectName"),
	...
	public ModelAndView editEntry2() {
		...
		if (command[0].equalsIgnoreCase("Work Flow")) {
			...
			return mav;
				...
				.equalsIgnoreCase("Work Flow of Project Team Members")) {
			...
			for (int i = 0; i < EbWFofPTM.size(); i++) {
				if (Integer.parseInt(command[1]) == EbWFofPTM.get(i)
						.getTaskId()) {
	...
	public ModelAndView infoEntry2() {
		...
		if (command[0].equalsIgnoreCase("Work Flow")) {
			...
			return mav;
				...
				.equalsIgnoreCase("Executive Project Portfolio Dashboard")) {
			mav = new ModelAndView("dashboard");

			for (int i = 0; i < EbDashboard.size(); i++) {
				if (command[1].equalsIgnoreCase(EbDashboard.get(i)
						.getProjectName())) {
			...
			if (iSchId <= 0) {
				...
				this.Ebdb
						.ExecuteUpdate("update X25Task set nmTaskFlag=2, dtStart=now(),dtTargetEnd="
								+ Ebdb.fmtDbString(fmtDateToDb(dtEnd))
								+ ",stURL="
								+ this.Ebdb.fmtDbString(stLink)
								+ " where RecId=" + iSchId);
			}

			this.Ebdb.ExecuteUpdate("delete from X25RefTask where nmTaskId="
	...
	/*
	 * Controller for testing public ModelAndView LogInProcess(
	 * 
	 * @RequestParam(value="stEmail", required=false, defaultValue="") String
	 * stEmail,@RequestParam(value="stPwd", required=false, defaultValue="")
	 * String stPwd, HttpSession session) {
	 * 
	 * 
	 ...
	 * 
	 * @SuppressWarnings("resource") ApplicationContext EbEpsContext = new
	 * ClassPathXmlApplicationContext("ebeps.xml"); EbClientTemplate EbcTemplate
	 * = (EbClientTemplate)EbEpsContext.getBean("EbClientTemplate");
	 * List<EbClient> EbC = EbcTemplate.clientMapping(stEmail, stPwd);
	 * 
	 * System.out.println(stEmail+","+stPwd); ModelAndView mav = null;
	 * 
	 * if(!EbC.isEmpty()){ // login
	 * 
	 * @SuppressWarnings("resource") ApplicationContext DbEpsContext = new
	 * ClassPathXmlApplicationContext("dbeps.xml"); EbUserDataTemplate
	 * EbUDTemplate =
	 * (EbUserDataTemplate)DbEpsContext.getBean("EbUserDataTemplate");
	 * List<EbUserData> EbUD = EbUDTemplate.mappingUserData(EbC.get(0));
	 * if(!EbUD.isEmpty()){ EbUserData EbUD1 = EbUD.get(0);
	 * System.out.println("Debug 0"); EbWorkFlowTemplate EbWFTemplate =
	 * (EbWorkFlowTemplate)DbEpsContext.getBean("EbWorkFlowTemplate");
	 * List<EbWorkFlow> EbWF = EbWFTemplate.mappingWorkFlow(EbUD.get(0));
	 * System.out.println("Debug 1"); session.setAttribute("client", EbUD1); mav
	 * = new ModelAndView("login"); mav.addObject("client", EbUD1); return mav;
	 * if(!EbWF.isEmpty()){ System.out.println("Debug2");
	 * mav.addObject("client", (Object)EbWF); System.out.println("Debug3");
	 * System.out.println(EbWF.get(0).getProjectName());
	 * 
	 * }else{ System.out.println("Null!!!!!!!!!!!!!!!!!"); } } }else{ // login
	 * with user question System.out.println("Illegal Login"); mav = new
	 * ModelAndView("greetme");
	 * 
	...
	public String makeTask(int iTriggerId, String stTaskDetail) {
		...
		int iU = 0;
		try {
			ResultSet rsTrigger = this.Ebdb.ExecuteSql("select * from Triggers"
	...
	public void sendMilestoneMessage(String stPrjId, int nmBaseline,
		...
		if ("Done".equals(stStatus) && (iFlag & 0x4000) != 0) {
			rs = this.Ebdb.ExecuteSql("SELECT * FROM `projects` WHERE `RecId`="
					+ stPrjId);
			rs.next();
