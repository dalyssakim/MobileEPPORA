...
public class EbMail {
	...
	public EbMail(EbUserData EbUD) {
		...
		this.ebNorm = new EbNormalized(EbUD);
	...
	public int sendMail(String stTo, String stToName, String stSubject,
			String stBodyIn, int nmCampaignId) throws Exception, MessagingException {
		int iPos = 0;
		int iDnc = 1;
		int i=0;
		...
		try {
			...
			if ((iPos > 0) && (iDnc == 0)) {
				...
				int nmUserId = this.ebNorm.getEmailId(stTo);
	...
	public int sendMailCL(String stTo, String stToName, String stSubject,
		...
		try {
			...
			if ((iPos > 0) && (iDnc == 0) && (iCL < 4)) {
				this.astUser = this.gastEmails[(this.giEmailIx++)]
						.split("\\|");
				if (this.giEmailIx >= this.gastEmails.length) {
					this.giEmailIx = 0;
				}
	...
	public int readEmail() throws Exception {
		int iReturn = 0;
		int nmUserId = 0;
			...
			for (int iM = 0; iM < messages.length; iM++) {
				try {
					...
					for (int iA = 0; iA < aaFrom.length; iA++) {
						...
						nmUserId = this.ebNorm.getEmailId(astFrom[0],
								astFrom[1]);
						if (nmUserId > 0) {
							iMaxComm++;
	...
	public int sendMail(ResultSet rsUser, String stCampaignId,
		...
		int iReturn = -1;
		EbCipher cCrypt = new EbCipher("yu#$%1xx6");
		String stTo = "";
		String stToName = "";
		String stFirstName = "";
		String stMiddleName = "";
		...
		try {
			...
			if (stTo != null) {
				...
				this.stBody = this.stBody.replace("~~stEMailEncode",
						URLEncoder.encode(cCrypt.encrypt(stTo), "UTF-8"));
